...
lf.aggregate <- function(LC,fcol,lcol,bins,new_bins) {
  # check data first
  ...
  LC_plot <- LC[,c("year","quarter","lat","lon",paste0(bins))]
  LC_long <- data.frame(tidyr::gather(LC,fcol:lcol,key = "length",value = "number"))
